//! Utilities for parsing Flattened Devicetree (DTB).
//!
//! This crate provides utilities for handling Flattened Devicetree (FDT)
//! structures, as described in the [Devicetree Specification].
//! It allows parsing, traversing, and extracting information from DTB blobs,
...
use self::{
    fdt::{Header, HeaderValidationError, ReserveEntry},
...
/// Represents a Flattened Devicetree (DTB) structure.
///
/// This struct holds references to each section of a DTB (structure block,
/// strings block, memory reservation entries). It allows safe traversal and
/// extraction of information from a DTB blob without copying the data.
...
impl Devicetree<'static> {
    ...
    /// # Safety
    ///
    /// The caller must ensure that `addr` points to a valid DTB in memory.
    ///
    /// Returns an error if the DTB is invalid or the address is not aligned.
